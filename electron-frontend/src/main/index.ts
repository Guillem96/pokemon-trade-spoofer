...
process.on('SIGINT', () => {
  console.log('Stopping python server with SIGTERM', pythonProcess.kill('SIGTERM'));
  if (process.platform !== 'darwin') {
    app.quit();
  }
  process.exit();
...
ipcMain.handle('start-bgb-server', async (event, party: Party): Promise<boolean> => {
  try {
...
ipcMain.handle('stop-bgb-server', async (): Promise<boolean> => {
  try {
